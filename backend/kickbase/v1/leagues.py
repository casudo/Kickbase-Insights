...
def user_feed(token: str, league_id: str, user_id: str):
    """
    ### Get the user feed.

    USER FEED TYPES (from user feed):
    - Type 12: Verkauft an Kickbase
    - Type 16: ?
    - Type 26: ?
    ...
    ```
    """
    url = f"https://api.kickbase.com/leagues/{league_id}/users/{user_id}/feed?filter=12&start=0"
    headers = {
        "Content-Type": "application/json",
        "Accept": "application/json",
        "Cookie": f"kkstrauth={token};",
    ...
    ### Send GET request to get the user feed
    try:
        response = requests.get(url, headers=headers).json()
    except:
        raise exceptions.NotificatonException("Notification failed! Please check your Discord Webhook URL.") # TODO: Change exception
